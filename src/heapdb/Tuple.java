...
public class Tuple {
	 ...
	 * @param schema the schema for the typle
	 * @param values the values for the respective columns
	 */
	...
	/**
	 * Copy constructor
	 * @param record the typle to copy
	 */
	public Tuple(Tuple record) {
		this.schema=record.schema;
		this.values = new Object[schema.size()];
		for (int k=0; k<schema.size(); k++) {
			this.values[k] = record.values[k];
		}
	}
